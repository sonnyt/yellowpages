...
(function( exports ) {
    ...
    function YellowPages() {
        this.url = 'http://www.yellowpages.com/{{location}}/{{term}}?page={{page}}&s={{sort}}';
    }

    YellowPages.prototype.search = function( options, callback ) {
        if ( !options.term || !options.location ) {
            return;
        }

        this._scrape( options, function( result ) {
            if ( !!( callback && callback.constructor && callback.call && callback.apply ) ) callback( result );
        });
    };

    YellowPages.prototype._scrape = function( options, callback ) {
